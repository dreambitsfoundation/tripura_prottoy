var states = [
    ...
    function getCities(state){
        ...
        for (var i = 0; i<cities.length; i++){
            if(cities[i]["state"] === state){
                temp.push(cities[i]["name"]);
            }
        }
        return temp;
    }

    function select_first(i){
        if (i==0){
            return "selected";
        }
        return "";
    ...
    function change_cities(state){
        var all_cities = getCities(state);
        var citiesHtml = "";
        for (var i = 0; i<all_cities.length; i++){
            citiesHtml += "<option value='" + all_cities[i] + "'>" + all_cities[i] + "</option>";
        }
...
for (var i = 0; i<all_states.length; i++){
    statesHtml += "<option value='" + all_states[i] + "'>" + all_states[i] + "</option>";
}
change_cities(all_states[0]);
$("#states").html(statesHtml);
$("#states").change(function(){
    var selected_state = $(this).val();
    change_cities(selected_state);
})
